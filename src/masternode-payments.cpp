...
bool IsBlockPayeeValid(const CBlock& block, int nBlockHeight)
{
    ...
    // Entire budget/superblock logic removed permanently.
    // SCH does not use governance or superblocks.

    // Governance, superblock, and budget logic has been permanently removed from SCH.
    // Only standard masternode payment validation remains.
    ...
    // SPORK_8 has been permanently removed; always enforce masternode payment validation unconditionally.
    return false;
}
...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake)
{
    ...
    // SCH does not use governance or superblocks.
    // Always use standard masternode payment logic.
...
bool CMasternodePayments::ProcessBlock(int nBlockHeight)
{
    ...
    int n = mnodeman.GetMasternodeRank(activeMasternode.vin,
                                       nBlockHeight - 100,
                                       ActiveProtocol());
    ...
    // pay to the oldest MN that still had no payment but its input is old enough
    // and it was active long enough
    int nCount = 0;
    CMasternode* pmn =
        mnodeman.GetNextMasternodeInQueueForPayment(nBlockHeight, true, nCount);
    ...
    if (pmn != NULL) {
        ...
        CScript payee =
            GetScriptForDestination(pmn->pubKeyCollateralAddress.GetID());
    ...
    if (!CMessageSigner::GetKeysFromSecret(strMasterNodePrivKey,
                                           ...
                                           pubKeyMasternode)) {
        LogPrint("masternode",
                 "CMasternodePayments::ProcessBlock() - Error upon calling GetKeysFromSecret.\n");
        return false;
    }
