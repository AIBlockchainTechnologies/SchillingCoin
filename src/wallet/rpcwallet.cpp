...
UniValue CreateColdStakeDelegation(const UniValue& params, CWalletTx& wtxNew, CReserveKey& reservekey)
{
    ...
    // Backwards-compatible flag: allow callers to force creation even if runtime flag disables coldstaking.
    bool fForceNotEnabled = false;
    if (params.size() > 5 && !params[5].isNull())
        fForceNotEnabled = params[5].get_bool();

    // SPORK_17_COLDSTAKING_ENFORCEMENT has been removed from runtime.
    // Respect the runtime -coldstaking flag so operators/tests can opt out if needed.
    if (!IsColdStakingEnabled() && !fForceNotEnabled) {
        std::string errMsg = "Cold Staking is disabled via -coldstaking.\n"
                "You may force the stake delegation by setting the fForceNotEnabled parameter to true.\n"
                "WARNING: If relayed before network-wide support, this tx may be rejected by peers.\n";
